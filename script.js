...
function startTest() {
    ...
    curIdx = 0;
    score = 0;
    answered = false; // Жауап берілмеген күйді қалпына келтіру
    timeLeft = 1800;
    showPage('page-test');
    renderQ();
    startTimer();
...
function renderQ() {
    answered = false; // Әр сұрақ сайын жауап берілмеген күйді қалпына келтіру
    ...
    order.forEach(function(origIdx, pos) {
        ...
        // Әр батырмаға жеке event listener қосу
        btn.addEventListener('click', function(event) {
            // Бұл жерде event объектісін қолдануға болады, бірақ қажет емес
            if (!answered) {
                answered = true; // Жауап берілді деп белгілеу
...
function handleAnswer(selIdx, corrIdx) {
    ...
    setTimeout(function() {
        ...
        if (curIdx >= shuffled.length) {
            ...
            renderQ(); // Келесі сұрақты көрсету
        }
    }, ok ? 1100 : 1700);
}
